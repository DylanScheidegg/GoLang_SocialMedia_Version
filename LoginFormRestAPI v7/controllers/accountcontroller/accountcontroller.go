...
func LoadPosts(frList string) []showPost {
	...
	for _, post := range posts {
		uPosts = append(uPosts, showPost{Fname: post[0], Lname: post[1], Text: post[2], Date: post[3], Time: post[4]})
	}
...
func FetchPosts(response http.ResponseWriter, request *http.Request) {
	...
	// execute the sql statement
	row := db.QueryRow(`SELECT * FROM users WHERE id=$1 AND email=$2 RETURN friends`, id, ema)

	// unmarshal the row object to user
	err := row.Scan(&user.Friends)

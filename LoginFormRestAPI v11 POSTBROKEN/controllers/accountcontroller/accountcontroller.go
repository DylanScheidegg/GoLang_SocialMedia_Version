...
func LoadPosts(frList string) []showPost {
	...
	for _, fr := range newList {
		...
		var countPosts int
		db.QueryRow(`SELECT COUNT(*) FROM posts WHERE uID=$1`, fr).Scan(&countPosts)
...
func Register(response http.ResponseWriter, request *http.Request) {
	...
	case sql.ErrNoRows: // NO USER FOUND WITH GIVEN EMAIL
		var id int
		db.QueryRow(`SELECT COUNT(*) FROM users`).Scan(&id)
...
func PostPost(response http.ResponseWriter, request *http.Request) {
	...
	// execute the sql statement
	row := db.QueryRow(`SELECT * FROM users WHERE id=$1 AND email=$2`, id, email)
	...
	case nil:
		...
		fmt.Println(request.Form.Get("postData"))
		fmt.Println(request.PostFormValue("postData"))
		//fmt.Println(request.FormFile())
		post := request.PostFormValue("postData")

		var pid int
		db.QueryRow(`SELECT COUNT(*) FROM posts`).Scan(&pid)
		pid = pid + 2
		dt := time.Now().Format("01-02-2006 15:04:05")
		newDT := strings.Split(dt, " ")
		date := newDT[0]
		time := newDT[1]

		fmt.Println(pid, id, post, date, time)

		// execute the sql statement to INSERT
		err2 := db.QueryRow(`INSERT INTO posts(id, uid, text, date, time) VALUES ($1, $2, $3, $4, $5)`, pid, id, post, date, time)

		if err2 != nil {
			log.Fatalf("Unable to execute the query. %v", err2)
		}

		fmt.Printf("Inserted a single record %v", pid)

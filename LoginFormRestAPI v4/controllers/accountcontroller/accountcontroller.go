...
func LoadPosts(frList []uint8) {
	for das, element := range frList {
		// element is the element from frList for where we are
		fmt.Println(das, element)
	}
}
